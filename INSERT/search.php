  ...
  <head>
	...
	if (isset($_POST['amenities'])) {
		...
		if(1 == $N){
			...
			$sqlAmenities= "Room_id in $sqlAmenities1 INTERSECT $sqlAmenities2 INTERSECT $sqlAmenities3 INTERSECT  $sqlAmenities4 ";
	...
	if (empty($_POST["arrival"]) or empty($_POST["departure"])) {
		echo "Missing arrival -departure";
		$sqlRented = TRUE; }
	else {
	...
	if(empty($_POST["min_price"]) and empty($_POST["max_price"])){
			echo "Missing min price and max price ";
			$sqlPrice=TRUE;
		}
	...
	if(empty($_POST["total_room_number"])){
		echo "Missing total room number";
